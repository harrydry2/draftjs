...
class Profile extends Component {
	...
	async typeAhead (e) {
		...
		if (res.data.fullName === 'Woops!!') {
			this.setState({i: 2});
		} else if (res.data.profileImage === 'https://abs.twimg.com/sticky/default_profile_images/default_profile_400x400.png' && res.data.backgroundImage === `url('undefined')` && res.data.fullName !== 'Woops!!') {
			this.setState({i: 15});
		}
		else if (res.data.profileImage === 'https://abs.twimg.com/sticky/default_profile_images/default_profile_400x400.png' && res.data.fullName !== 'Woops!!') {
			this.setState({i: 16});
		}
	...
	componentDidUpdate() {
		...
		if (this.props.lastClickedReducer.lastClicked === 'FAVIMG' && this.state.i !== 6 && this.state.favImgCounter < 1) {
			let counter = this.state.favImgCounter + 1
			this.setState({
				i: 6,
		...
		if (this.props.lastClickedReducer.lastClicked === 'FAVCHANGED' && this.state.i !== 7 && this.state.favChangedCounter < 2) {
			let counter = this.state.favChangedCounter + 1
			this.setState({
				i: 7,
		...
		if (this.props.lastClickedReducer.lastClicked === 'top' && this.state.i !== 13 && this.state.i !== 0 && this.state.i !== 1 && this.state.i !== 2 && this.state.i !== 15 && this.state.i !== 16 && this.state.i !== 17) {
			this.setState({i: 13})
		}
				...
				<div className="permalinkHeader">
					<div className="accountGroup">
						...
						<span className="fullNameGroup">
							<strong className="fullName" onClick={(e) => this.focusUsername(e)}>{fullName}</strong>
						...
						<form className="search" onSubmit={(e) => this.typeAhead(e)} onClick={(e) => this.lastClickedTop(e)} >
